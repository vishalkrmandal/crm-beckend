...
// @desc    Get trade commission data
// @route   GET /api/ibclients/ib-configurations/trade-commissions
// @access  Private (Client)
exports.getTradeCommissions = async (req, res) => {
    try {
        ...
        // Get all commission trades for this IB
        const commissionTrades = await IBCommission.find({
            ibConfigurationId: ibConfiguration._id
        })
        ...
        const trades = commissionTrades.map((trade, index) => ({
            acNo: `18064${6 + index}`, // Generate AC numbers similar to screenshot
            openTime: trade.createdAt,
            closeTime: new Date(trade.createdAt.getTime() + (Math.random() * 300000)), // Random close time within 5 mins
            openPrice: (0.6400 + Math.random() * 0.0050).toFixed(4), // Random price around 0.64
            closePrice: (0.6400 + Math.random() * 0.0050).toFixed(4),
            symbol: 'AUDUSD.s', // Default symbol
            profit: trade.baseAmount || 0,
            volume: 0.0100, // Default volume
            rebate: trade.commissionAmount || 0,
            status: 'completed'
        }));
        ...
        const totals = {
            ...
            totalRebate: trades.reduce((sum, trade) => sum + trade.rebate, 0)
        };
        ...
        res.status(500).json({
            success: false,
            message: 'An error occurred while fetching trade commission data.'
        });
    }
};
